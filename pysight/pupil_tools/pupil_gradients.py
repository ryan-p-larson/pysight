...
class PupilGradients(object):
    def __init__(self, fast_width=25.0, inv_intensity_weight_divisor=100):
        """Helper class.
    ...
    def get_center_map(self, eye_img_grey):
        ...
        grad_x_img = cv.Sobel(eye_img_grey, ddepth=cv.CV_32F, dx=1, dy=0, ksize=5)
        grad_y_img = cv.Sobel(eye_img_grey, ddepth=cv.CV_32F, dx=0, dy=1, ksize=5)
        ...
        # Invert image for weighting + then normalize values
        ...
        for y in range(eye_img_grey.shape[0]):
            for x in range (eye_img_grey.shape[1]):
                ...
                accumulator += self.__test_possible_centers_formula(x, y,
                        darkness_weights[y][x], grad_x_img[y][x], grad_y_img[y][x],
